import discord from discord.ext import commands from discord import app_commands import json import os
...
@bot.tree.command(name="商品追加", description="商品を追加") async def add_product(interaction: discord.Interaction): class AddModal(discord.ui.Modal, title="商品追加"): name = discord.ui.TextInput(label="商品名") price = discord.ui.TextInput(label="値段") desc = discord.ui.TextInput(label="説明", style=discord.TextStyle.paragraph) contents = discord.ui.TextInput(label="商品内容(1行=1在庫 / 空で無限)", style=discord.TextStyle.paragraph, required=False)
...
@bot.tree.command(name="在庫", description="在庫確認") async def stock(interaction: discord.Interaction): data = load_data() text = ""

for name, info in data["products"].items():
    if info.get("infinite"):
        s = "∞"
    else:
        s = len(info.get("items", []))
    text += f"{name} : {s}\n"
...
for name, info in data["products"].items():
    embed.add_field(name=f"{name} ({info['price']})", value=info["desc"], inline=False)
